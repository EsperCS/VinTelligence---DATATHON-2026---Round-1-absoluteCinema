...
COMPARISON_PATH = DATA_DIR / "conversion_regime_model_comparison.csv"
VALIDATION_PREDICTIONS_PATH = DATA_DIR / "conversion_regime_validation_predictions.csv"
FEATURE_IMPORTANCE_PATH = DATA_DIR / "conversion_regime_feature_importance.csv"
REPORT_PATH = LOG_DIR / "conversion_regime_model_report.txt"
LOG_FILE = LOG_DIR / "train_conversion_regime_model.log"
...
FEATURE_COLUMNS = CALENDAR_FEATURES + PROMOTION_FEATURES + CONVERSION_LAG_FEATURES + TRAFFIC_FEATURES
...
def setup_logging(log_file: Path = LOG_FILE) -> logging.Logger:
    ...
    logger = logging.getLogger("train_conversion_regime_model")
...
def build_static_features(df: pd.DataFrame, logger: logging.Logger) -> pd.DataFrame:
    ...
    promo_all = orders_base.build_promotion_features(df[DATE_COL], orders_base.PROMOTIONS_PATH, logger)
    promo = promo_all[[DATE_COL] + PROMOTION_FEATURES].copy()
    ...
    fill_columns = PROMOTION_FEATURES
...
def run() -> None:
    ...
    reporter.emit("Conversion-Rate Regime Window Experiment")
    reporter.emit("======================================")
    reporter.emit("")

    reporter.emit("1. Load data and build conversion model table")
    ...
    reporter.emit(
        "Evaluation assumption: reconstructed orders use known 2022 web_sessions to isolate the quality of "
        "conversion-rate prediction, since no future sessions model is introduced in this experiment."
    )
